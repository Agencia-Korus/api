...
def upgrade() -> None:
    ...
    op.alter_column('entregavel', 'descricao',
    ...
    op.drop_index(op.f('idx_portfolio_destaque'), table_name='portfolio', postgresql_where='(destaque = true)')
...
def downgrade() -> None:
    ...
    op.create_index(op.f('idx_tarefa_responsavel'), 'tarefa', ['responsavel_id'], unique=False)
    ...
    op.create_index(op.f('idx_projeto_cliente'), 'projeto', ['cliente_id'], unique=False)
    op.create_index(op.f('idx_portfolio_destaque'), 'portfolio', ['destaque'], unique=False, postgresql_where='(destaque = true)')
    ...
    op.create_index(op.f('idx_historico_funcionario'), 'historico_xp', ['funcionario_id', sa.literal_column('data DESC')], unique=False)
    op.create_index(op.f('idx_entregavel_servico'), 'entregavel', ['servico_id'], unique=False)
    op.alter_column('entregavel', 'descricao',
               existing_type=sa.String(length=500),
               type_=sa.VARCHAR(length=255),
               existing_nullable=False)
    op.create_index(op.f('idx_consentimento_usuario'), 'consentimento_lgpd', ['usuario_id'], unique=False)
    op.create_index(op.f('idx_comunicado_alvo'), 'comunicado', ['alvo'], unique=False)
    op.create_index(op.f('idx_comentario_tarefa'), 'comentario', ['tarefa_id'], unique=False)
    op.create_index(op.f('idx_anexo_tarefa'), 'anexo', ['tarefa_id'], unique=False)
    op.create_index(op.f('idx_academy_publicado'), 'academy', ['publicado'], unique=False)
    # ### end Alembic commands ###
