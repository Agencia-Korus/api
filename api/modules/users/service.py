    ...
    async def create(self, payload: UsuarioCreate) -> Usuario:
        ...
        if payload.role == UserRole.CLIENTE:
            ...
            cliente = Cliente(
                ...
                if cliente_payload else usuario.nome,
                cnpj_cpf=cliente_payload.cnpj_cpf
                if cliente_payload else f'api-{usuario.id}',
                segmento=cliente_payload.segmento
                if cliente_payload else None
            )
        ...
        if payload.role == UserRole.FUNCIONARIO:
            ...
            funcionario = Funcionario(
                ...
                if funcionario_payload else 'Funcionário',
                especialidade=funcionario_payload.especialidade
                if funcionario_payload else None,
            )
